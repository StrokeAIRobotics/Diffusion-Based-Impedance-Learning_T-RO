...
% ============================================================
% ------------------- FIRST MJT: Start → touch ---------------
% ============================================================
q_first = deg2rad([11.66, 62.77, -0.01, -91.88, 60.04, 19.11, -18.11]');
...
for k = 1:length(t)
    tau = t(k)/T_first;
    pos(:,k)  = mjt(p_ini, p_first, tau);
    quat(:,k) = quatinterp(q_ini_quat', q_first_quat', ...
...
% ============================================================
q_fin = deg2rad([9.63, 71.49, 13.26, -75.31, -19.12, 37.20, 62.92]');
H_fin = robot.getForwardKinematics(q_fin, 'bodyID', 7);
q_fin_quat = rotm2quat(H_fin(1:3,1:3))';
p_fin = H_fin(1:3,4);
p_fin(3) = p_fin(3) - 0.07;   % z correction
...
for k = 1:length(t)
    tau = t(k)/T_fin;
    pos(:,k)  = mjt(p_start, p_fin, tau);
    quat(:,k) = quatinterp(q_start', q_fin_quat', ...
                           10*tau^3 - 15*tau^4 + 6*tau^5,'slerp')';
end
...
pos = zeros(3,length(t));
quat = repmat(q_traj(:,end),1,length(t));

for k = 1:length(t)
    tau = t(k)/T_up;
    pos(:,k) = mjt(p_start, p_goal_up, tau);
end
...
output_file = '/YOUR_FOLDER/pegInHole_square_fullTrajectory.txt';
